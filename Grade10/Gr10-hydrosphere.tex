...
\begin{tabular}{cc}
	...
	\begin{note}
      {note: }The total mass of the hydrosphere is approximately $1,4~{\times}{10}^{18} ~~ \mathsf{tonnes}$! (The volume of one tonne of water is approximately 1 cubic metre (this is about 900 A4 textbooks!))
	\end{note}
	\end{minipage}
	\end{tabular}
	\par
...
\label{m38138*uid13}\item Water is seldom 'pure'. It usually has lots of things dissolved (e.g. ${\mathsf{Mg}}^{2+}$, ${\mathsf{Ca}}^{2+}$ and $\mathsf{NO}_{3}^{-}$ ions) or suspended (e.g. soil particles, debris) in it. Where do these substances come from?
...
The hydrosphere provides an important place for many animals and plants to live. Many gases (e.g. ${\mathsf{CO}}_{2}$, ${\mathsf{O}}_{2}$), nutrients e.g. nitrate ($\mathsf{NO}_{3}^{-}$), nitrite ($\mathsf{NO}_{2}^{-}$) and ammonium ($\mathsf{NH}_{4}^{+}$) ions, as well as other ions (e.g. ${\mathsf{Mg}}^{2+}$ and ${\mathsf{Ca}}^{2+}$) are dissolved in water. The presence of these substances is critical for life to exist in water.
...
In chapter~\ref{chap:rxnsaq} we saw how compounds can dissolve in water. Most of these compounds (e.g. ${\mathsf{Na}}^{+}$, ${\mathsf{Cl}}^{-}$, ${\mathsf{Ca}}^{2+}$, ${\mathsf{Mg}}^{2+}$, etc.) are safe for humans to consume in the small amounts that are naturally present in water. It is only when the amounts of these ions rise above the safe levels that the water is considered to be polluted.
\par 
