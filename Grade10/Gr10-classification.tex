...
{In groups of 3 or 4 look at the labels of medicines, food items, and any other items that you use often. What can you tell about the material inside the container from the list of ingredients? Why is it important to have a list of ingredients on the materials that we use? Do some research on the safety data of the various compounds in the items that you looked at. Are the compounds in the items safe to use? In the food items, what preservatives and additives are there? Are these preservatives and additives good for you? Are there natural (from plants) alternatives? What do different indigineous people groups use to flavour and preserve their food? }  \end{activity}\par \label{m38708*cid2}
            \subsection{ Mixtures}
            \nopagebreak
...
\end{pspicture}
\end{center}
\caption{A microscopic representation of a heterogeneous mixture. The gray circles are one substance (e.g. sand) and the white circles are another substance (e.g. water)}
\end{figure}
...
\begin{table}[h!]
 \begin{center}
  \begin{tabular}{|l|l|l|}\hline
   ...
   solid-liquid & Suspension & sand in water \\ \hline
   liquid-gas & Aerosol & hair spray \\ \hline
   gas-solid & Foam & styrofoam \\ \hline
  \end{tabular}
...
\end{pspicture}
\end{center}
\caption{A microscopic representation of a homogeneous mixture. The gray circles are one substance (e.g. salt) and the white circles are another substance (e.g. water)}
\end{figure}
...
%NTS add diagrams here for the wexes to explain concept better
%      \noindent
%      \hspace*{-30pt}\includegraphics[width=0.5in]{col11305.imgs/pspencil2.png}   \raisebox{25mm}{   
%      \begin{mdframed}[linewidth=4, leftmargin=40, rightmargin=40]  
...
{\westep{Now we name them} For a) we list the potassium first and the permanganate ion second. So a) is potassium permanganate. For b) we list the ammonium ion first and change the ending of chlorine to -ide. So b) is ammonium chloride.}}
%\end{enumerate}}
    \end{wex}
%    \end{mdframed}
...
{\westep{list the names} In part a) the charge on sodium is $+1$ and the charge on sulphate is $-2$, so we must have two sodiums for every sulphate. In part b) the charge on potassium is $+1$ and the charge on chromate is $-2$, so we must have two potassiums for every chromate.}
{\westep{write the answer} a) is ${\mathrm{Na}}_{2}{\mathrm{SO}}_{4}$ and b) is ${\mathrm{K}}_{2}{\mathrm{CrO}}_{4}$}}
    \end{wex}
%    \end{mdframed}
%    }
%    \noindent
...
%take properties from a later chapter that mentions them
...
%\label{m38708*eip-id1170755988427}\begin{enumerate}[noitemsep, label=\textbf{\alph*}. ] 
 %           \leftskip=20pt\rightskip=\leftskip
\westep{Use what you know} \begin{enumerate}
