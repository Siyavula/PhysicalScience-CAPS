...
Quantity: Amplitude (A) \hspace{2cm} Unit name: metre \hspace{2cm} Unit symbol: $m$} 
...
\IFact{
A tsunami is a series of sea waves caused by an underwater earthquake, landslide, or volcanic eruption. When the ocean is deep, tsunamis may be less than a 30~cm high on the ocean'sÃÂÃÂÃÂÃÂs surface and can travel at speeds up to 700~$\text{km}\cdot \text{hr}^{-1}$.
...
Quantity: Period ($T$) \hspace{2cm} Unit name: second \hspace{2cm} Unit symbol: s  } 
...
Quantity: Frequency ($f$) \hspace{2cm} Unit name: hertz \hspace{2cm} Unit symbol: Hz } 
...
\end{wex}
    ...
    \noindent
	  ...
	  Quantity: Wave speed ($v$) \hspace{2cm} Unit name: metre per second \hspace{2cm} Unit symbol: $\text{m}\cdot \text{s}^{-1}$
         } 
