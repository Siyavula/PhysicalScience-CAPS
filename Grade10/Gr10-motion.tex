...
\end{minipage}
       ...
       \label{m38787*id62541}\begin{enumerate}[noitemsep, label=\textbf{\arabic*}. ] 
            \label{m38787*uid1}\item \textbf{position} or \textbf{displacement} which tells us about an objects location or change of location
...
\IFact{The \textbf{jerk} is the name we give to how fast the acceleration is changing.}
    ...
    \label{m38787*cid3}
            ...
            The first important fact to know when studying motion, is you have to know where you are. The word \textsl{position} describes your location (where you are). However, saying that you are \textsl{here} or \textsl{there} is meaningless, and you have to use known points (reference points) to specify your position \textsl{relative} to a known reference point. 

\begin{minipage}{.5\textwidth}
      \label{m38787*id62597}For example, if you are 2~m from the doorway, inside your classroom then your reference point is the doorway. This defines your position inside the classroom. Notice that you need a reference point (the doorway) and a direction (inside) to define your location.
\end{minipage}
\begin{minipage}{.5\textwidth}
\begin{center}
