...
\Definition{Distance}{Distance is the total length of the path taken in going from the initial position, $\vec{x}_{i}$, to the final position, $\vec{x}_{f}$. Distance is a scalar.\\Symbol: $D$\hspace{2cm} S.I. Unit: $\text{m}$}
...
We have looked at the average velocity and speed but sometimes we want to be more precise about what is happening between the initial and final points in a problem.
...
\end{figure}      
\par
\Definition{Gradient} {The gradient of a line can be calculated by dividing the change in the $y$-value (dependent variable) by the change in the $x$-value (independent variable). $m = \frac{\Delta y}{\Delta x}$ \par  } 
...
\Tip{The gradient of a velocity vs. time graph gives the acceleration.}
    ...
    &= \frac{{v}_{f}-{v}_{i}}{{t}_{f}-{t}_{i}}\hfill \\ 
