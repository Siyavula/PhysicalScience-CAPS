...
of \textbf{negative} electric charge.\par \label{m38745*eip-222}The electrons determine the charge on an atom. If the number of electrons is the same as the number of protons then the atom will be neutral. If the number of electrons is greater than the number of protons then the atom will be negatively charged. If the number of electrons is less than the number of protons then the atom will be positively charged. Atoms that have a charge (positive or negative) are called ions. Ions will be covered in more detail in a later chapter. For example: a neutral sodium atom can lose one electron to become a positively charged sodium atom ($\mathsf{Na}^{+}$). A neutral chlorine atom can gain one electron to become a negatively charged chlorine ion ($\mathsf{Cl}^{-}$)\par 
      \label{m38745*uid11}
...
denoted as:\par 
    ...
    \begin{equation*}
    _{26}^{56}\mathsf{Fe}\phantom{\rule{4pt}{0ex}}
      \end{equation*}
...
{%Answer
\westep{Give the element symbol} $\mathsf{Na}$
\westep{Find the number of protons} Sodium has 11 protons, so we have: ${}_{11}\mathsf{Na}$
...
\westep{Write the answer} In standard notation sodium is given by: $_{11}^{23}\mathsf{Na}$. The number of protons is 11, the number of neutrons is 12 and the number of electrons is 11.
}    
\end{wex}
...
element symbol $\mathsf{C}$ (see the Table of Elements). Atoms which have the same number of protons, but a different number of neutrons, are called \textbf{isotopes}.\par 
...
isotopes of an element are the same, but they might vary in how stable their nucleus is. Note that we can also write elements as $\mathsf{X\; -\; A}$ where the X is the element symbol and the A is the atomic mass of that element. For example $\mathsf{Cl-}35$ has an atomic mass of $35 u$ (17 protons and 18 neutrons), while $\mathsf{Cl-}37$ has an atomic mass of $37 u$ (17 protons and 20 neutrons).\par 
...
\end{figure}
\end{minipage}
\Tip{The spectroscopic electron configuration can be written in shorter form. This form is written as [noble gas]electrons, where the noble gas is the nearest one that occurs before the element. For example, magnesium can be represented as $\mathsf{[Ne]}3\mathsf{s}^{2}$ and carbon as $\mathrsf{[H]}2\mathsf{s}^{2}2\mathsf{p}^{2}$. This is known as the condensed electron configuration.}
\begin{wex}
{%title
Aufbau diagrams and spectroscopic electron configuration 
}
{%question
Give the electron configuration for nitrogen ($\mathsf{N}$) and draw an aufbau diagram.
}
{%answer
\westep{Give the number of electrons} Nitrogen has 7 electrons.
\westep{Place two electrons in the $1\mathsf{s}$ orbital} We start by placing two electrons in the $1\mathsf{s}$ orbital: ${1\mathsf{s}}^{2}$. 
\begin{figure}[H]
\scalebox{0.7}{
...
\westep{Place two electrons in the $2\mathsf{s}$ orbital} 
We put two electrons in the $2\mathsf{s}$ orbital: ${2\mathsf{s}}^{2}$. 
    \setcounter{subfigure}{0}
\begin{figure}[H]
\scalebox{0.7}{
 \begin{pspicture}(-8,-6)(8,-2)
...
\westep{Place three electrons in the $2\mathsf{p}$ orbital}
We place 3 electrons in the $2\mathsf{p}$ orbital: ${2\mathsf{p}}^{3}$.
\begin{figure}[H]
\scalebox{.7}{
 \begin{pspicture}(-8,-8)(8,5)
%\psgrid
...
\westep{Write the final answer} The electron configuration is: ${1\mathsf{s}}^{2}{2\mathsf{s}}^{2}{2\mathsf{p}}^{3}$. The Aufbau diagram is given in the step above.
...
{%question
Give the electron configuration for ($\mathsf{O}^{2-}$) and draw an Aufbau diagram.
}
{%answer
...
\westep{Place six electrons in the $2\mathsf{p}$ orbital}
We place 6 electrons in the $2\mathsf{p}$ orbital: ${2\mathsf{p}}^{6}$.
\westep{Write the final answer} The electron configuration is: ${1\mathsf{s}}^{2}{2\mathsf{s}}^{2}{2\mathsf{p}}^{6}$. The Aufbau diagram will look as follows:
\begin{figure}[H]
\scalebox{.7}{
 \begin{pspicture}(-8,-8)(8,5)
%\psgrid
