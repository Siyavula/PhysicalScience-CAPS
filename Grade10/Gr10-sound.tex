...
Sound waves need something to travel through. Usually they travel through air, but they can travel much faster and farther through a string. The string has to be tight or else the sound wave canât travel through it. The cup helps to amplify the sound on the other end. 
\end{activity}
...
\Definition{Speed of sound } { \label{m38799*meaningfhsst!!!underscore!!!id164}
...
In a space with many small objects there are reflections at every surface but they are too small and too mixed up to have an outcome that a human can hear. However, when there is an open space that has only large surfraces, for example a school hall that is empty, then the reflected sound can actually be heard. The sound wave is reflected in such a wave that the wave looks the same but is moving in the opposite direction.

This means that if you stand in a hall and loudly say ``hello'' you will hear youself say ``hello'' a split second later. This is an echo. This can also happen outdoors in a wide open space with a large reflecting surface nearby, like standing near a mountain cliff in an area with no trees or bushes.
...
The most common oscilloscope controls are for amplitude, frequency, triggering, and channels. Once your teacher has helped you acquire a signal using the correct channel and triggering you will use the amplitude and frequency controls to display the characterstics of the sound being produced.\\
...
\label{m38800*uid24}\item Ultrasound c
an be used to form pictures of things we cannot see, like unborn babies or tumors.
...
Velocity & $v$ & $\text{m}\cdot\text{s}^{-1}$ &  $\text{m}\cdot\text{s}^{-1}$ \\ \hline
Wavelength & $\lambda$ & m & m \\ \hline
Amplitude & $A$ & m & m \\ \hline
Period & $T$ & s & s \\ \hline
