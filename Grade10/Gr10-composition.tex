...
\label{m38120*id307099}The answer lies in the smallest building block of  matter: the \textbf{atom}. It is the \textsl{type} of atoms, and the way in which they are \textsl{arranged} in a material, that affects the properties of that substance. This is similar to building materials. We can use bricks, steel, cement, wood, straw (thatch), mud and many other things to build structures from. Just as the choice of building material affects the properties of the structure, so does the choice of atoms affect the properties of matter.\par 
\end{minipage}
...
\Definition{ Compound} { \label{m38120*meaningfhsst!!!underscore!!!id74}
      A compound is a group of two or more different atoms that are 
attracted to each other by relatively strong forces or bonds. The atoms are combined in definite proportions. 
       } 
...
A \textbf{chemical formula} is an abbreviated (shortened) way of describing a molecule, or some other chemical substance. In chapter~\ref{chap:classification}, we saw how chemical compounds can be represented using element symbols from the Periodic Table. A chemical formula can also tell us the \textsl{number} of atoms of each element that are in a molecule and their \textsl{ratio} in that molecule.

 \Definition{Chemical formula} { \label{m38120*meaningfhsst!!!underscore!!!id87}
The chemical formula is a concise way of expressing information about the atoms that make up a particular chemical compound. The chemical formula gives the exact number of each type of atom in the molecule. } 

For example in figure~\ref{fig:representing isobutane} the chemical formula of 2-methylpropanone is $\text{C}_{4}\text{H}_{10}$. This tells us that there are $4$ carbon atoms and $10$ hydrogen atoms in this compound, i.e the ratio of carbon to hydrogen is $4:10$. But we can simplify this ratio to: $2:5$. This gives us the \textbf{empirical formula} of the compound. 
...
\end{pspicture}
\caption{Diagram showing (a) the chemical, (b) the empirical and (c) the structural formula of 2-methylpropanone}
...
In this model, the bonds between atoms are shown as 'sticks'. These 'sticks' are colored to show which atoms are bonding.
...
\item candle
\item matches
\item copper sulfate ($\text{CuSO}_{4}\cdot 10\textsf{H}_{2}\textsf{O}$)
\item zinc metal
...
\textsl{Reaction 3}
\begin{enumerate}[label=\textbf{\arabic*}.]
\item Place a spatula full of copper sulfate crystals into a test tube and heat the tube over a bunsen burner.
\item Record your observations.
\end{enumerate}
...
\label{m38120*uid69}\item \textbf{Covalent molecular structures} interact and exist as seperate molecules.
...
the elements in the compound. This formula gives the exact number of atoms of each element in the compound.
...
\label{m38120*uid75}\item A \textbf{ball-and-stick} model is a 3-dimensional molecular model that uses 'balls' to represent atoms and 'sticks' to represent the bonds between them.
